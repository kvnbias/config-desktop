...
while true; do
  read -p "
Choose the closest packman repository to install. If the repository install failed, Try other repositories.
  ...
  case $pr in
    ...
    [1] )
      if echo "$os" | grep -q 'tumbleweed'; then
        ...
        sudo zypper ar -cfp 90 http://ftp.gwdg.de/pub/linux/misc/packman/suse/openSUSE_Leap_$version/Essentials packman-essentials
      fi
    [2] )
    [3] )
    [4] )
