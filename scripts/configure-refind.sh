              ...
              case $blcstmztn in
                [1] )
                  ...
                  while true; do
                    ...
                    case $prttn in
                      * )
                        if lsblk -i -o $outputs | grep 'part' | grep 'ext4' | grep -v -e 'swap' -e '/' | grep -q "$prttn "; then
                          sudo mkdir -p /mnt-refind
                          echo "Mounting /dev/$prttn"
                          sudo mount "/dev/$prttn" "/mnt-refind"

                          entryname="Entry $parttn"
                          while true; do
                          ...
                          kernelparams=
                          uuid=$(lsblk -i -o $outputs | grep 'part' | grep 'ext4' | grep -v -e 'swap' -e '/' | grep "$parttn " 2> /dev/null | head -1 | cut -f 11 -d ' ')
                          ...
                          generate_menu_entry "$entryname" "$icpath" "/boot/$loader" "/boot/$initrd" "\"$kernelparams\""
                ...
                [2] )
                  ...
                  uuid=$(sudo blkid | grep $root | cut -f 2 -d ' ')
                  if [[ $uuid == *"LABEL"* ]]; then
                    uuid=$(sudo blkid | grep $root | cut -f 3 -d ' ')
                  fi
